...
class EndlessPagination(BasePagination):
    ...
    # queryset is no loner a queryset, it could be a list because of the cache
    ...
    def paginate_hbase(self, hb_model, row_key_prefix, request):
        if 'created_at__gt' in request.query_params:
            ...
            # note: we use max_timestamp because if we use none, the serialized data will be (1, none) -> 1, it will be
            # at the top
            ...
            if len(objects) and objects[0].created_at == int(created_at__gt):
                # [1,2,3,4] => [4,3,2]
                # it's like new_list = origin_list[start: stop: direction]
                # empty means it starts with the tail, 0 means end with the head (not included)
                objects = objects[:0:-1]
            else:
                objects = objects[::-1]
            self.has_next_page = False
        ...
        if 'created_at__lt' in request.query_params:
            ...
            # 由于 hbase 只支持 <= 的查询而不支持 < （包含start,stop), 因此我们还需要再多取一个 item 保证 < 的 item 有 page_size + 1 个
            created_at__lt = request.query_params['created_at__lt']
            start = (*row_key_prefix, created_at__lt)
            stop = (*row_key_prefix, None)
            # we set the stop instead of none because we want the stop has the prefix info, otherwise, it will get the
            # data with unexpected prefix. Note, the row key is like (1, ts1), (1,ts2), (1, ts3), (2, ts1)
    ...
    # to check whether cache list can meet the pagination requirement. If no, we will return none
    def paginate_cached_list(self, cached_list, request):
        ...
        if self.has_next_page:
            return paginated_list
        # 如果 cached_list 的长度不足最大限制，说明 cached_list 里已经是所有数据了, 本来就是没有下一页了
